    ...
    public class AuthService : MonoBehaviour
    {
        ...
        private void OnDestroy()
        {
            if (_userChannel != null)
            {
                ...
                {
                    Debug.LogWarning($"Error unsubscribing channel: {ex.Message}");
                }
                _userChannel = null;
            }
        }
        ...
        public async UniTask<AppUser> SignInAsync(string email, string password)
        {
            ...
            if (session != null && session.User != null)
            {
                ...
                // }
                // 移除本地防重检查，因为 SetCurrentUser 可能已经被 SessionListener 触发了
        ...
        private async UniTask StartListenForKick(string userId, string currentToken)
        {
            ...
            {
                Debug.LogError($"Failed to subscribe to kick channel: {ex.Message}");
                // 订阅失败不应导致程序崩溃，只是无法监听被顶号
            }
        }
