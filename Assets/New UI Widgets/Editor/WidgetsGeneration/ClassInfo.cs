...
namespace UIWidgets.WidgetGeneration
{
	...
	public class ClassInfo
	{
		...
		/// </summary>
		public ClassField AutocompleteField;
		...
		readonly Comparison<ClassField> FieldsComparison = (ClassField x, ClassField y) =>
		{
			return -(x.ActualFieldType == typeof(string)).CompareTo(y.ActualFieldType == typeof(string));
		};
		...
		void Process(Type type)
		{
			...
			var ns = type.GetCustomAttribute<GeneratorNamespaceAttribute>();
			if (ns != null)
			{
				WidgetsNamespace = ns.Widgets;
				WidgetsEditorNamespace = ns.Editor;
			}
			else
			{
				var settings = WidgetsReferences.Instance;
				var type_namespace = string.IsNullOrEmpty(Namespace)
					? "UIWidgets.Custom." + ShortTypeName + "NS"
					: Namespace;

				WidgetsNamespace = settings.GetWidgetsNamespace(type_namespace);
				WidgetsEditorNamespace = settings.GetWidgetsEditorNamespace(type_namespace);
			}

			var paths = type.GetCustomAttribute<GeneratorPathsAttribute>();
			if (paths != null)
			{
				Paths = new DataPath(paths);
			}
		...
		ClassField FindAutocompleteField(List<ClassField> fields)
		{
			foreach (var field in fields)
			{
