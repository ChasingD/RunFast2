...
namespace UIWidgets
{
	...
	public class ScriptingDefineSymbol : ISetting
	{
			...
			get
			{
				if (!Available)
				{
					return string.IsNullOrEmpty(PackageName)
						? "Not installed."
						: string.Format("{0} is not installed", PackageName);
				}
		...
		public ScriptingDefineSymbol(string symbol, Func<bool> installed, Action<bool> onChanged = null, string enabledText = "Enabled", string disabledText = "Disabled", string package = null)
		{
			Symbol = symbol;
			available = installed;
			OnChanged = onChanged;
