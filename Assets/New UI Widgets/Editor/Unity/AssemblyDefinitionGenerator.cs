	...
	public class AssemblyDefinitionGenerator
	{
		readonly struct AssemblyDefinition
		{
			...
			readonly string Content()
			{
				...
				return string.Format(ReferenceGUID.Instance.AsmdefTemplate, Name, References, platforms);
			}
		}
		...
		public static void Create()
		{
			...
			if (string.IsNullOrEmpty(ReferenceGUID.Instance.ScriptsFolder)
				|| string.IsNullOrEmpty(ReferenceGUID.Instance.EditorFolder)
				|| string.IsNullOrEmpty(ReferenceGUID.Instance.AsmdefTemplateFile))
			{
			...
			assemblies.Add("R3.Unity");
			var editor = new AssemblyDefinition("UIWidgets.Editor", true, assemblies);

			main.Save(ReferenceGUID.Instance.ScriptsFolder);
			editor.Save(ReferenceGUID.Instance.EditorFolder);

			if (!string.IsNullOrEmpty(ReferenceGUID.Instance.SamplesFolder))
			{
				samples.Save(ReferenceGUID.Instance.SamplesFolder);
			}
		}
		...
		{
			Delete(ReferenceGUID.Instance.ScriptsFolder);
			Delete(ReferenceGUID.Instance.SamplesFolder);
			Delete(ReferenceGUID.Instance.EditorFolder);
