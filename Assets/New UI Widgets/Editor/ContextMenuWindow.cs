...
namespace UIWidgets
{
	...
	public class ContextMenuWindow : TreeListEditorWindow
	{
		...
		/// <inheritdoc/>
		protected override SerializedObject TargetSerialized => GetTargetSerialized<Menu.ContextMenu>();
		...
		/// <inheritdoc/>
		protected override DisplayInfo Add(int index, DisplayInfo info)
		{
			var result = base.Add(index, info);
			var menu_item = List.GetArrayElementAtIndex(index + 1);

			if (menu_item != null)
			{
				var visible = menu_item.FindPropertyRelative("Visible");
				visible.boolValue = true;

				var interactable = menu_item.FindPropertyRelative("Interactable");
