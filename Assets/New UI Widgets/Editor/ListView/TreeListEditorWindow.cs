...
namespace UIWidgets
{
	...
	public abstract class TreeListEditorWindow : EditorWindow
	{
		...
		/// </summary>
		protected readonly GUILayoutOption[] AddButtonOptions = new GUILayoutOption[] { GUILayout.ExpandWidth(true) };
		...
		protected virtual void OnGUI()
		{
			...
			{
				info = Add(List.arraySize - 1, info);
			}
		...
		protected virtual void FixTreeList(int count)
		{
			...
			{
				var serialized_depth = List.GetArrayElementAtIndex(i).FindPropertyRelative("Depth");
				if (serialized_depth.intValue < 0)
				{
					serialized_depth.intValue = 0;
				}

				if ((serialized_depth.intValue - depth) > 1)
				{
					serialized_depth.intValue = depth + 1;
				}

				depth = serialized_depth.intValue;
			}
		}
		...
		protected virtual DisplayInfo ChangeDepth(int index, DisplayInfo info, int deltaDepth)
		{
			...
			var serialized_depth = List.GetArrayElementAtIndex(index).FindPropertyRelative("Depth");
			var depth = serialized_depth.intValue;
			for (int j = index + 1; j < info.Count; j++)
			{
				var child = List.GetArrayElementAtIndex(j);
				if (child.FindPropertyRelative("Depth").intValue <= depth)
			...
			serialized_depth.intValue += deltaDepth;
