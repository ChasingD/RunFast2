...
namespace UIWidgets.Styles
{
	...
	public class StyleImagePropertyDrawer : PropertyDrawer
	{
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (IsOpened)
			{
				...
				if (EditorGUI.EndChangeCheck())
				{
					...
					if (new_sprite != null)
					{
						var old_type = (Image.Type)image_type.intValue;
						if (new_sprite.border.SqrMagnitude() > 0)
						{
							image_type.intValue = (int)Image.Type.Sliced;
						}
						else if (old_type == Image.Type.Sliced)
						{
							image_type.intValue = (int)Image.Type.Simple;
						}
					}
				}
				...
				if (sprite.objectReferenceValue is Sprite)
				{
					...
					DrawImageProperties(position, (Image.Type)image_type.intValue, property);
				}
		...
		protected Rect DrawImageProperties(Rect position, Image.Type imageType, SerializedProperty property)
		{
			...
			switch (imageType)
			{
				...
				case Image.Type.Filled:
					...
					var fill_method = (Image.FillMethod)fill_method_property.intValue;
		...
		protected float ImagePropertiesHeight(Image.Type imageType, SerializedProperty property)
		{
			var result = 0f;
			var fill_method_property = property.FindPropertyRelative("fillMethod");
			var fill_origin = property.FindPropertyRelative("fillOrigin");
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
		{
			...
			if (sprite.objectReferenceValue is Sprite)
			{
				...
				result += ImagePropertiesHeight((Image.Type)image_type.intValue, property);
			}
