namespace UIWidgets.Examples
{
	...
	public class TestListViewEnum : MonoBehaviour
	{
		...
		void ValueChanged(int index)
		{
			...
			{
				Debug.Log(string.Format("selected: {0}", EnumHelper<AdditionalCanvasShaderChannels>.Instance.ToString(WrapperWithFlags.Selected)));
			}
			else if (Wrapper != null)
			{
				Debug.Log(string.Format("selected: {0}", EnumHelper<ListViewType>.Instance.ToString(Wrapper.Selected)));
			}
			else if (WrapperObsolete != null)
			{
				Debug.Log(string.Format("selected: {0}", EnumHelper<TestEnumObsolete>.Instance.ToString(WrapperObsolete.Selected)));
			}
		}
