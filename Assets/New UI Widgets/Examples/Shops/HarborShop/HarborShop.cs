namespace UIWidgets.Examples.Shops
{
	...
	public class HarborShop : MonoBehaviourInitiable, IUpgradeable
	{
		...
		/// Prices.
		/// </summary>
		protected Prices ItemsPrices = new Prices();
		...
		ObservableList<HarborOrderLine> CreateOrderLines(Trader harbor, Trader player)
		{
			...
			foreach (var item in harbor.Inventory)
			{
				...
				var order_line = new HarborOrderLine(
					item,
					ItemsPrices.GetPrice(item, harbor.PriceFactor),
					ItemsPrices.GetPrice(item, player.PriceFactor),
					item.Quantity,
		...
		Item FindItemByName(ObservableList<Item> items, string name)
		{
