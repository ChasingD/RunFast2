...
// Should be used only "_Width", "_Height", "_HalfSizeAndOrigin", "_InternalHalfSizeAndOrigin", "_BorderRadius", "_BorderColor", and "_BorderWidth" shader properties,
		...
		CGPROGRAM
			...
			v2f vert(appdata_t v)
			{
				...
				if (_UIVertexColorAlwaysGammaSpace)
				{
					if(!IsGammaSpace())
					{
						v.color.rgb = GammaToLinearSpace3(v.color.rgb);
					}
				}
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				float4 color = IN.color * (tex2D(_MainTex, IN.texcoord.xy) + _TextureSampleAdd);
				float2 pos = IN.texcoord.zw - float2(0.5f, 0.5f);

				float alpha = RectangleAlpha(pos, _Size.xy, _HalfSizeAndOrigin, _BorderRadius, 0.0f);
				float border_alpha = _BorderWidth > 0 ? RectangleAlpha(pos, _Size.xy, _InternalHalfSizeAndOrigin, _BorderRadius, _BorderWidth) : 1.0f;
				...
				#if defined(UIWIDGETS_MULTIPLY_ON_ALPHA)
				color.rgb *= color.a;
				#endif
