...
Shader "Custom/New UI Widgets/GradientShaderPlaneHSV"
{
	Properties
	{
		...
		_ColorLeft ("Color Left", Vector) = (1,1,1,1)
		// HSV color on the right side
		_ColorRight ("Color Right", Vector) = (1,1,1,1)
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			inline float4 HSVtoRGB_linear(in float4 hsv)
			{
				float4 result = HSVtoRGB(hsv);
				if (!IsGammaSpace())
				{
					result.rgb = GammaToLinearSpace3(result.rgb);
				}
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				color = color * HSVtoRGB_linear(lerp(_ColorLeft, _ColorRight, IN.texcoord.x) + lerp(_ColorBottom, _ColorTop, IN.texcoord.y));
