...
Shader"Custom/New UI Widgets/UIBorderDrawer"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			float lines_alpha(float2 pos)
			{
				float result = (_Borders.x > 0 && pos.x > 0 && pos.x < _Borders.x)
					+ (_Borders.y > 0 && pos.x > (1.0 - _Borders.y) && pos.x < 1)
					+ (_Borders.z > 0 && pos.y > (1.0 - _Borders.z) && pos.y < 1)
					+ (_Borders.w > 0 && pos.y > 0 && pos.y < _Borders.w);
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				float l = lines_alpha(IN.texcoord.zw);
