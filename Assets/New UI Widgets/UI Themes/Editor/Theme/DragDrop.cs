...
namespace UIThemes.Editor
{
	...
	public class DragDrop : PointerManipulator
	{
		...
		void ShowIndicator(int index)
		{
			...
			{
				element = target.ElementAt(index - 1);
			}

			Vector2 pos = Vector2.zero;
			if (horizontal)
			{
				pos.x = element.worldBound.x - target.worldBound.x - indicator.layout.width;
			}
			else
			{
				pos.y = element.worldBound.y - target.worldBound.y - indicator.layout.height;
			}

			#if UNITY_6000_2_OR_NEWER
			indicator.style.translate = pos;
			#else
