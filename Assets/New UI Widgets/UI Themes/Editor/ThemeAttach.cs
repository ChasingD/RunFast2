...
namespace UIThemes.Editor
{
	...
	public class ThemeAttach
	{
		...
		public class TypesCache
		{
			...
			/// </summary>
			public readonly Dictionary<Type, RequireOptionMethod> RequireOptionCache = new Dictionary<Type, RequireOptionMethod>();

			/// <summary>
			/// Cache of methods to find option by value.
			/// </summary>
			public readonly Dictionary<Type, RequireOptionMethod> FindOptionCache = new Dictionary<Type, RequireOptionMethod>();

			/// <summary>
			/// Cache of methods to get property value.
			/// </summary>
			public readonly Dictionary<Type, PropertyGetterMethod> PropertyGetterCache = new Dictionary<Type, PropertyGetterMethod>();

			/// <summary>
			/// Cache of SetValueMethod.
			/// </summary>
			public readonly Dictionary<Type, SetValueMethod> SetValueCache = new Dictionary<Type, SetValueMethod>();

			/// <summary>
			/// Cache of methods to check if should attach property value.
			/// </summary>
			public readonly Dictionary<Type, PropertyAttachValueMethod> PropertyAttachValueCache = new Dictionary<Type, PropertyAttachValueMethod>();
		}

		/// <summary>
		/// Cache.
		/// </summary>
		protected TypesCache Cache => EditorStaticFields.Instance.ThemeAttachCache;
		...
		public delegate OptionId RequireOptionMethod(Theme.IValuesWrapper values, VariationId variationId, object value, string optionName);
		...
		public delegate object PropertyGetterMethod(Target target);
		...
		public delegate bool SetValueMethod(Theme.IValuesWrapper values, VariationId variationId, OptionId optionId, object value);
		...
		public delegate bool PropertyAttachValueMethod(Target target);
		...
		protected virtual SetValueMethod SetValue(Theme.IValuesWrapper values)
		{
			...
			if (Cache.SetValueCache.TryGetValue(type, out var set_value))
			{
				return set_value;
			}
			...
			Cache.SetValueCache[type] = set_value;
		...
		{
			var type = values.GetType();
			if (Cache.RequireOptionCache.TryGetValue(type, out var require))
			{
				return require;
			}
			...
			Cache.RequireOptionCache[type] = require;
		...
		{
			var type = values.GetType();
			if (Cache.FindOptionCache.TryGetValue(type, out var require))
			{
				return require;
			}
			...
			Cache.FindOptionCache[type] = require;
		...
		{
			if (Cache.PropertyGetterCache.TryGetValue(property.ValueType, out var getter))
			{
				return getter;
			}
			...
			Cache.PropertyGetterCache[property.ValueType] = getter;
		...
		{
			if (Cache.PropertyAttachValueCache.TryGetValue(property.ValueType, out var getter))
			{
				return getter;
			}
			...
			Cache.PropertyAttachValueCache[property.ValueType] = getter;
