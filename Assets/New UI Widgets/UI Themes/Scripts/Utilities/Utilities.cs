namespace UIThemes
{
	...
	public static class Utilities
	{
		...
		public class TypesMethods
		{
			...
			void AssemblyStaticMethods(Assembly assembly, List<MethodInfo> methods)
			{
				Type[] types;
				try
				{
			...
			void TypeStaticMethods(Type type, List<MethodInfo> methods)
			{
			...
			readonly Dictionary<Type, List<MethodInfo>> StaticMethodsOfAttribute = new Dictionary<Type, List<MethodInfo>>();

			/// <summary>
			/// Invoke static methods with the specified attributes.
			/// </summary>
			/// <typeparam name="TAttribute">Type of attribute.</typeparam>
			public void InvokeStaticMethods<TAttribute>()
			...
			List<MethodInfo> FilterStaticMethods<TAttribute>(List<MethodInfo> methods)
				where TAttribute : Attribute
			{
			...
			readonly Dictionary<Tuple<Type, Type, Type>, List<MethodInfo>> GenericStaticMethodsOfAttribute = new Dictionary<Tuple<Type, Type, Type>, List<MethodInfo>>();
			...
			public void InvokeStaticMethods<TAttribute, TArgument0, TArgument1>(TArgument0 args0, TArgument1 args1)
				where TAttribute : Attribute
			{
			...
			List<MethodInfo> FilterStaticMethods<TAttribute, TArgument0, TArgument1>(List<MethodInfo> methods)
				where TAttribute : Attribute
			{
				var result = new List<MethodInfo>();
			...
			bool IsValidMethod<TAttribute, TArgument0, TArgument1>(MethodInfo method)
				where TAttribute : Attribute
			{
				if (method.GetCustomAttribute<TAttribute>() == null)
				{
		...
		public static void InvokeStaticMethods<TAttribute>()
			where TAttribute : Attribute
			=> StaticFields.Instance.TypesMethods.InvokeStaticMethods<TAttribute>();
		...
		public static void InvokeStaticMethods<TAttribute, TArgument0, TArgument1>(TArgument0 args0, TArgument1 args1)
			where TAttribute : Attribute
			=> StaticFields.Instance.TypesMethods.InvokeStaticMethods<TAttribute, TArgument0, TArgument1>(args0, args1);
		...
		public static bool IsWhiteSprite(Sprite sprite)
		{
			...
			if (HasLabel(path, StaticFields.Instance.SpritesLabelWhite))
			{
				return true;
			}
			...
			if (StaticFields.Instance.SpritesWhite.TryGetValue(id, out var cache))
			{
			...
			StaticFields.Instance.SpritesWhite[id] = new Tuple<bool, DateTime>(is_white, time);
		...
		public static bool IsExcludedSprite(Sprite sprite)
		{
			...
			return HasLabel(path, StaticFields.Instance.SpritesLabelExclude);
#endif
		}
