namespace UIWidgets
{
		...
		where T : MonoBehaviour, ITemplatable
	{
		...
		readonly Predicate<T> RemoveNull = x => x == null;
		...
		public T Instance(string name)
		{
			...
			{
				var original = templates[name];
				template = Compatibility.Instantiate(original);
				Utilities.FixInstantiated(original, template);
