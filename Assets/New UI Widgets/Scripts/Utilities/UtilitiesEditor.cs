namespace UIWidgets
{
	...
	{
		static Dictionary<Type, string> Types2FriendlyName => StaticFields.Instance.Types2FriendlyName;

		static Dictionary<string, Type> TypesCache => StaticFields.Instance.TypesCache;
		...
		{
			if (!Types2FriendlyName.TryGetValue(type, out var result))
			{
				result = GetFriendlyTypeNameInternal(type);
				Types2FriendlyName[type] = result;
			}

			return result;
		}

		static string GetFriendlyTypeNameInternal(Type type)
		{
			var friendly_name = type.Name;

			if (!type.IsGenericType)
			{
				var namespace_prefix = string.IsNullOrEmpty(type.Namespace) ? string.Empty : (type.Namespace + ".");
				var prefix = type.DeclaringType == null
					? namespace_prefix
					: GetFriendlyTypeName(type.DeclaringType) + ".";

				return prefix + friendly_name;
			}
			...
			{
				var type_parameter_name = type_parameters[i].IsGenericParameter
					? type_parameters[i].Name
					: GetFriendlyTypeName(type_parameters[i]);
				if (i > 0)
				{
					sb.Append(',');
				}

				sb.Append(type_parameter_name);
			}
