namespace UIWidgets
{
	...
	public static class Compatibility
	{
		...
		public static HashSet<string> GetScriptingDefineSymbols(BuildTargetGroup targetGroup)
		{
			...
			foreach (var symbol in symbols.Split(StaticFields.Instance.ScriptingSymbolsSeparator))
			{
				if (symbol.Length > 0)
				{
					result.Add(symbol);
		...
		public static void SetScriptingDefineSymbols(BuildTargetGroup targetGroup, HashSet<string> symbols)
		{
			...
			symbols.CopyTo(arr);
			var str = string.Join(StaticFields.Instance.ScriptingSymbolsSeparator[0].ToString(), arr);
