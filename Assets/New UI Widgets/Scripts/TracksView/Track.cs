namespace UIWidgets
{
	...
	[Serializable]
	public class Track<TData, TPoint> : ObservableDataWithPropertyChanged
		...
		where TPoint : IComparable<TPoint>
	{
		...
		protected void CalculateOrder()
		{
			...
			for (int i = 1; i < SortedData.Count; i++)
			{
				...
				if (InRangeExclusive(point, start, end))
				{
		...
		{
			return InRangeInclusive(item, VisibleStart, VisibleEnd);
		}

		/// <summary>
		/// Is data located between specified points including start and end.
		/// </summary>
		/// <param name="item">Item.</param>
		/// <param name="startPoint">Start point.</param>
		/// <param name="endPoint">End point.</param>
		/// <returns>true if data located between specified points; otherwise false.</returns>
		protected bool InRangeInclusive(TData item, TPoint startPoint, TPoint endPoint)
		{
		...
		/// <summary>
		/// Is data located between specified points without start and end.
		/// </summary>
		/// <param name="item">Item.</param>
		/// <param name="startPoint">Start point.</param>
		/// <param name="endPoint">End point.</param>
		/// <returns>true if data located between specified points; otherwise false.</returns>
		protected bool InRangeExclusive(TData item, TPoint startPoint, TPoint endPoint)
		{
			var start = (startPoint.CompareTo(item.StartPoint) <= 0)
				|| (startPoint.CompareTo(item.EndPoint) < 0);
