namespace UIWidgets.Menu
{
	...
	/// Menu item.
	/// </summary>
	public class MenuItem : ObservableDataWithPropertyChanged
	{
		...
		public HotKey HotKey
		{
			...
			set
			{
				if (value == null)
				{
					throw new ArgumentNullException(nameof(value), "HotKey cannot be null");
				}

				if (Change(ref hotKey, value))
				{
					DestroyInputAction();
					CreateHotKeyAction();
				}
		...
		/// Items changed.
		/// </summary>
		protected void ItemsChanged() => Changed(nameof(Items));
		...
		protected virtual string Key2String(Key key)
		{
			return key switch
			{
				...
				Key.Digit8 => "8",
				Key.Digit9 => "9",
				_ => EnumHelper<Key>.Instance.ToString(key),
			};
		}
