...
#pragma warning restore 0414
		...
		/// Active menu.
		/// </summary>
		protected static List<ContextMenu> ActiveMenu => StaticFields.Instance.ContextMenuActive;
		...
		/// Has visible items?
		/// </summary>
		/// <returns>true if has visible items; otherwise false.</returns>
		...
		public virtual void Open(Vector2 position)
		{
			if (!HasVisibleItems())
			{
				Debug.LogWarning("Menu cannot be opened because it does not have any visible items", this);
				return;
			}
		...
		/// </summary>
		protected static int LastToggledByKey
		{
			get => StaticFields.Instance.ContextMenuLastToggledByKey;

			set => StaticFields.Instance.ContextMenuLastToggledByKey = value;
		}
		...
		{
			get => StaticFields.Instance.ContextMenuCurrent;

			set => StaticFields.Instance.ContextMenuCurrent = value;
		}
		...
		{
			if (LastToggledByKey == WidgetsTime.Instance.FrameCount)
			{
				return;
			}
			...
			LastToggledByKey = WidgetsTime.Instance.FrameCount;
		}
		...
		{
			var menu = FindMenu(StaticFields.Instance.ContextMenuIsOpen);
			if (menu != null)
			{
				return menu;
			}
			...
			return FindMenu(StaticFields.Instance.ContextMenuIsDefault);
		}

		/// <summary>
		/// Menu comparison.
		/// </summary>
		protected static Comparison<ContextMenu> MenuComparison => StaticFields.Instance.ContextMenuComparison;
		...
		{
			get => StaticFields.Instance.ContextMenuToggleAction;

			set => StaticFields.Instance.ContextMenuToggleAction = value;
		}
		...
		protected virtual void CreateToggleAction()
		{
			if (ToggleAction == null)
			{
				...
				ToggleAction.AddBinding(string.Format("<Keyboard>/{0}", EnumHelper<Key>.Instance.ToString(Key.ContextMenu)));
