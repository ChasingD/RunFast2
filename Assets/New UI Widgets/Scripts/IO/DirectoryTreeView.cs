namespace UIWidgets
{
	...
	public class DirectoryTreeView : TreeViewCustom<DirectoryTreeViewComponent, FileSystemEntry>
	{
		...
		{
			return Path.GetFullPath(new Uri(path).LocalPath).TrimEnd(StaticFields.Instance.PathSeparators);
		}
		...
		public virtual bool SelectDirectory(string directory) => SelectDirectory(directory, true);
