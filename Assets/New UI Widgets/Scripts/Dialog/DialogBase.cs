...
#pragma warning restore 0649
		...
		/// Opened base dialogs.
		/// </summary>
		public static IReadOnlyList<DialogBase> OpenedBaseDialogs => WindowInstances<DialogBase>.Instance.Opened;
		...
		{
			add => WindowInstances<DialogBase>.Instance.OnOpen += value;

			remove => WindowInstances<DialogBase>.Instance.OnOpen -= value;
		}
		...
		{
			add => WindowInstances<DialogBase>.Instance.OnClose += value;

			remove => WindowInstances<DialogBase>.Instance.OnClose -= value;
		}
			...
			RectTransform parent = null)
		{
			...
			var canvas_root = SetCanvas(canvas, parent);

			var modal_root = parent != null ? parent : canvas_root;
			SetModal(modal, modalSprite, modalColor, modal_root);
		...
		public virtual DialogBase ShowAsync(
			...
			RectTransform parent = null)
		{
			Show(title, message, buttons, focusButton, position, icon, modal, modalSprite, modalColor, canvas, content, parent: parent);
		...
		/// Instance opened.
		/// </summary>
		protected virtual void InstanceOpened() => WindowInstances<DialogBase>.Instance.Add(this);

		/// <summary>
		/// Instance closed.
		/// </summary>
		protected virtual void InstanceClosed() => WindowInstances<DialogBase>.Instance.Remove(this);
		...
		public virtual RectTransform SetCanvas(Canvas canvas, RectTransform parent, bool worldPositionStays = true)
		{
			var parent_canvas = canvas != null
				? canvas.transform as RectTransform
				: UtilitiesUI.FindTopmostCanvas(parent != null ? parent : gameObject.transform);
			if (parent == null)
			{
			...
			if (parent == null)
			{
				Debug.LogError("Dialog is not in Canvas. Specify canvas or parent in the Show() method or in the SetCanvas() method.");
				return null;
			}

			DialogPosition.Restore();
			DialogPosition = HierarchyPosition.SetParent(transform, parent, worldPositionStays);
		...
		/// Is asynchronous?
		/// </summary>
		public bool IsAsync => Continuation != null;
