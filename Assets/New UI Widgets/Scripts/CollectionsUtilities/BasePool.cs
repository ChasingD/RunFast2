namespace UIWidgets
{
		...
		where T : Component
	{
		...
		readonly Predicate<T> IsNull = x => x == null;
		...
		public T Get()
		{
			return Get(template.transform.parent);
		}
		...
		public T Get(Transform parent)
		{
			...
			else
			{
				...
				if (!UnityObjectComparer<Transform>.Instance.Equals(parent, instance.transform.parent))
				{
		...
		{
			if (!UnityObjectComparer<T>.Instance.Equals(Template, pool.Template))
			{
				return false;
			}
