namespace UIWidgets
{
	...
	public class ObservableList<T> : IObservable, IObservableList<T>, IList<T>, IReadOnlyList<T>
	{
		...
		OnChange collectionItemChangedDelegate;

		PropertyChangedEventHandler itemPropertyChangedDelegate;

		static bool IsValueType => ObservableInfo<T>.Instance.IsValueType;

		static bool IsItemsObservable => ObservableInfo<T>.Instance.IsObservable;

		static bool IsItemsSupportNotifyPropertyChanged => ObservableInfo<T>.Instance.IsNotifyPropertyChanged;
		...
		void InitSubscription()
		{
			...
			collectionItemChangedDelegate = CollectionItemChanged;
			itemPropertyChangedDelegate = ItemPropertyChanged;
		}
		...
		void AddCallbacks(IEnumerable<T> items)
		{
			if (IsItemsObservable)
			{
				foreach (var item in items)
				{
					if (!IsNull(item))
					{
						(item as IObservable).OnChange += collectionItemChangedDelegate;
					}
				}
			}
			else if (IsItemsSupportNotifyPropertyChanged)
			{
				foreach (var item in items)
				{
					if (!IsNull(item))
					{
						(item as INotifyPropertyChanged).PropertyChanged += itemPropertyChangedDelegate;
					}
				}
			}
		}
