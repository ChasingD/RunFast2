namespace UIWidgets
{
	...
	public struct ColorHSV : IEquatable<ColorHSV>
	{
		...
		{
			var hue = Mathf.Abs(color.H / (float)HueMax);
			var saturation = Mathf.Abs(color.S / 255f);
			var value = Mathf.Abs(color.V / 255f);

			hue = Mathf.Clamp(hue, 0.000f, 1f);
			saturation = Mathf.Clamp(saturation, 0.000f, 1f);
			value = Mathf.Clamp(value, 0.000f, 1f);
			...
			return ihue switch
			{
				0 => new Color(value, t, p, a),
				1 => new Color(q, value, p, a),
				2 => new Color(p, value, t, a),
				3 => new Color(p, q, value, a),
				4 => new Color(t, p, value, a),
				_ => new Color(value, p, q, a),
			};
		}
