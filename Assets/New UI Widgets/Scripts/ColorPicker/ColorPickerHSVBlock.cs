namespace UIWidgets
{
	...
	public class ColorPickerHSVBlock : ColorPickerBlock
	{
		...
		protected virtual void UpdateViewReal()
		{
			...
			var h = currentColorHSV.H / 360f;
			var s = currentColorHSV.S / 255f;
			var v = currentColorHSV.V / 255f;

			if (hSliderBackground != null)
			{
				hSliderBackground.material.SetVector(StaticFields.Instance.ShaderColorLeft, new Vector4(0, 1, 1, 1));
				hSliderBackground.material.SetVector(StaticFields.Instance.ShaderColorRight, new Vector4(1, 1, 1, 1));
				hSliderBackground.SetMaterialDirty();
			}

			if (sSliderBackground != null)
			{
				sSliderBackground.material.SetVector(StaticFields.Instance.ShaderColorLeft, new Vector4(h, 0, Mathf.Max(ColorPicker.ValueLimit / 255f, v), 1));
				sSliderBackground.material.SetVector(StaticFields.Instance.ShaderColorRight, new Vector4(h, 1, Mathf.Max(ColorPicker.ValueLimit / 255f, v), 1));
				sSliderBackground.SetMaterialDirty();
			}

			if (vSliderBackground != null)
			{
				vSliderBackground.material.SetVector(StaticFields.Instance.ShaderColorLeft, new Vector4(h, s, 0, 1));
				vSliderBackground.material.SetVector(StaticFields.Instance.ShaderColorRight, new Vector4(h, s, 1, 1));
