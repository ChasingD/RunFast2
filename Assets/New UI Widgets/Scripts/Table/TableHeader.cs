namespace UIWidgets
{
	...
	public class TableHeader : UIBehaviourInteractable, IDropSupport<TableHeaderDragCell>, IPointerEnterHandler, IPointerExitHandler, IStylable, ILateUpdatable
	{
		...
		/// Cell size data.
		/// </summary>
		public readonly struct CellSizeData
		{
			/// <summary>
			/// Size.
			/// </summary>
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="CellSizeData"/> struct.
			/// </summary>
			/// <param name="size">Size.</param>
			/// <param name="layoutElement">LayoutElement.</param>
			public CellSizeData(float size, LayoutElement layoutElement)
			{
				Size = size;
		...
		/// <summary>
		/// Fill ContextMenu. Requires the ContextMenu component.
		/// </summary>
		[SerializeField]
		public bool FillContextMenu = true;

		/// <summary>
		/// Names for the ContextMenu items. They should be in the same order as columns.
		/// </summary>
		[EditorConditionBool(nameof(FillContextMenu))]
		public List<string> ContextMenuNames = new List<string>();

		bool contextMenuWarningShown;
		...
		/// Cells count.
		/// </summary>
		public int CellsCount => CellsInfo.Count;
		...
		/// Cell comparison.
		/// </summary>
		protected static Comparison<TableHeaderCellInfo> CellComparison => StaticFields.Instance.CellComparison;
...
#pragma warning restore 0618
		...
		protected virtual void LocaleChanged()
		{
			if (FillContextMenu)
			{
				// delay on one frame because names in cell headers can be not updated yet
				Updater.RunOnceNextFrame(this, UpdateContextMenuNames);
			}
		}
		...
		public virtual void SetCellsSize(IList<float> sizes, bool withListView = true)
		{
			...
			if (withListView)
			{
				using var _ = ListPool<CellSizeData>.Get(out var temp);
				for (int i = 0; i < sizes.Count; i++)
				{
					temp.Add(new CellSizeData(sizes[i], GetCell(i).LayoutElement));
				}

				List.SetTableSizes(temp);
			}
		...
		/// <summary>
		/// Reset the cells' width (or height).
		/// </summary>
		public virtual void ResetCellsSize() => SetCellsSize(-1f);

		/// <summary>
		/// Sets the cells' width (or height) to the same size.
		/// </summary>
		/// <param name="size">Size. It should be more or equal to the largest MinWidth (or MinHeight) of the LayoutElement in header cells.</param>
		public virtual void SetCellsSize(float size)
		{
			using var _ = ListPool<float>.Get(out var sizes);
			sizes.Capacity = Mathf.Max(sizes.Capacity, CellsCount);
...
#region IDropSupport<TableHeaderCell>
		...
		public virtual void Refresh()
		{
			...
			if (FillContextMenu && !UpdateContextMenu(ContextMenuNames) && !contextMenuWarningShown)
			{
				var msg = string.Format("ContextMenu is not found. You need to disable the FillContextMenu option or add the ContextMenu component to the TableHeader: {0}", name);
				Debug.LogWarning(msg, this);
				contextMenuWarningShown = true;
			}
		}
		...
		/// </summary>
		/// <param name="index">Index.</param>
		/// <returns>true if column visible; otherwise false.</returns>
		public virtual bool ColumnToggle(int index)
		{
			var target = CellsInfo[index];
			ColumnToggle(index, !target.Rect.gameObject.activeSelf);
			return target.Rect.gameObject.activeSelf;
		}
		...
		/// <summary>
		/// Build ContextMenu.
		/// Should be called only after ListView was inited.
		/// </summary>
		/// <param name="names">Menu items names.</param>
		/// <returns>true if menu was build; otherwise false.</returns>
		public virtual bool UpdateContextMenu(IList<string> names = null)
		{
			Init();

			if (!TryGetComponent<UIWidgets.Menu.ContextMenu>(out var menu))
			{
				return false;
			}

			names ??= ContextMenuNames;
			...
			foreach (var cell in CellsInfo)
			{
				var btn = Utilities.RequireComponent<ClickListener>(cell.Rect);
				btn.ClickEvent.AddListener(menu.OnPointerClick);

				var name = names != null && cell.Position < names.Count
					? names[cell.Position]
					: GetCellText(cell);
				...
				{
					Name = name,
					Checked = cell.Rect.gameObject.activeSelf,
					Tag = cell.Position,
				};
				item.Action.AddListener(ContextMenuToggle);
				items.Add(item);
			}
		...
		/// <summary>
		/// Update ContextMenu names.
		/// </summary>
		protected virtual void UpdateContextMenuNames() => UpdateContextMenuNames(ContextMenuNames);

		/// <summary>
		/// Update ContextMenu names.
		/// </summary>
		/// <param name="names">Names.</param>
		protected virtual void UpdateContextMenuNames(IList<string> names = null)
		{
			if (!TryGetComponent<UIWidgets.Menu.ContextMenu>(out var menu))
			{
				return;
			...
			{
				var cell = CellsInfo[i];
				items[i].Name = names != null && cell.Position < names.Count
					? names[cell.Position]
					: GetCellText(cell);
			}
		}
		...
		/// </summary>
		/// <param name="cell">Cell.</param>
		/// <returns>Text in the cell or cell name.</returns>
		protected virtual string GetCellText(TableHeaderCellInfo cell)
		{
			var ta = cell.Rect.GetComponentInChildren<TextAdapter>();
			if (ta != null)
			{
				return ta.text;
			}

			var text = cell.Rect.GetComponentInChildren<Text>();
			if (text != null)
			{
				return text.text;
			}
			...
			var tmpro = cell.Rect.GetComponentInChildren<TMPro.TextMeshProUGUI>();
			if (tmpro != null)
			{
				return tmpro.text;
			}
			...
			return cell.Rect.name;
		}

		/// <summary>
		/// Toggle column called by ContextMenu.
		/// </summary>
		/// <param name="item">Menu item.</param>
		protected virtual void ContextMenuToggle(MenuItem item) => item.Checked = ColumnToggle((int)item.Tag);
