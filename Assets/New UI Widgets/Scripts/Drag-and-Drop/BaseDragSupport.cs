	...
	public abstract class BaseDragSupport : UIBehaviourInitiable
	{
		...
		protected class Delay
		{
			...
			IEnumerator DragTimer()
			{
				...
				{
					yield return null;
					timer += WidgetsTime.Instance.DeltaTime(owner.UnscaledTime);
				}
		...
		/// Delay timer.
		/// </summary>
		protected Delay DelayTimer => delayTimer ??= new Delay(this);
		...
		/// The drag points.
		/// </summary>
		protected static Dictionary<InstanceID, RectTransform> DragPoints => StaticFields.Instance.DragSupportPoints;
