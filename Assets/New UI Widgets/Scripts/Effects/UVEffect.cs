namespace UIWidgets
{
	...
	public abstract class UVEffect : BaseMeshEffect, IMaterialModifier, IMeshModifier
	{
		...
		protected readonly struct MeshSize
		{
			...
			static Vector2 CalculateAspectRatio(Vector2 size, UVMode mode)
			{
				...
				return mode switch
				{
					...
					_ => throw new NotSupportedException(string.Format("Unknown UVMode: {0}", EnumHelper<UVMode>.Instance.ToString(mode))),
				};
			}
		...
		public override void ModifyMesh(VertexHelper vh)
		{
			...
			if ((size.x < 0f) || (size.y < 0f) || (vh.currentVertCount == 0))
			{
				return;
			}
			...
			switch (Channel)
			{
				...
				case UVChannel.UV1xy:
					...
					break;
				default:
					throw new NotSupportedException(string.Format("Unknown Channel: {0}", EnumHelper<UVChannel>.Instance.ToString(Channel)));
			}
		}
		...
		{
			var active = IsActive() && (graphic != null);
			if (!active)
			{
				return baseMaterial;
			}
			...
			EffectMaterial = new Material(baseMaterial)
			{
				shader = GetShader(),
			};
			InitMaterial();
		...
		protected virtual void ShaderChanged()
		{
			...
			EffectMaterial.shader = GetShader();
			UpdateMaterial();
		}
	}
}