	...
	public class RoundedCornersX4 : UVEffect
	{
		...
		protected override void SetMaterialProperties()
		{
			if (EffectMaterial != null)
			{
				...
				EffectMaterial.SetVector(StaticFields.Instance.ShaderCornersSize, size);
				EffectMaterial.SetVector(StaticFields.Instance.ShaderCornersX4HalfSizeAndOrigin, new Vector4(corners_half_size.x, corners_half_size.y, origin.x, origin.y));
				EffectMaterial.SetVector(StaticFields.Instance.ShaderCornersX4InternalHalfSizeAndOrigin, new Vector4(b_corners_half_size.x, b_corners_half_size.y, b_origin.x, b_origin.y));
				EffectMaterial.SetVector(StaticFields.Instance.ShaderCornersX4BorderRadius, radius);
				EffectMaterial.SetFloat(StaticFields.Instance.ShaderCornersX4BorderWidth, width);
				EffectMaterial.SetColor(StaticFields.Instance.ShaderCornersX4BorderColor, BorderColor);
			}
		}
		...
		protected Vector2 HalfSizeRotated(Vector2 size, BorderRadius radius)
		{
			...
			var width_half = Vector2.Dot(left_right, StaticFields.Instance.ShaderCornersX4WidthNormal) * .5f;

			var bottom_top = new Vector2(size.x, size.y - radius.BottomLeft - radius.TopRight);
			var height_half = Vector2.Dot(bottom_top, StaticFields.Instance.ShaderCornersX4HeightNormal) * .5f;
		...
		protected Vector2 Origin(Vector2 size, Vector2 halfSizeRotated, BorderRadius radius)
		{
			...
			var h_normal = StaticFields.Instance.ShaderCornersX4HeightNormal;
			var w_normal = StaticFields.Instance.ShaderCornersX4WidthNormal;

			return point
				+ (h_normal * Vector2.Dot(left2top, h_normal))
				+ (w_normal * halfSizeRotated.x)
				+ (h_normal * -halfSizeRotated.y);
		}
	}
}