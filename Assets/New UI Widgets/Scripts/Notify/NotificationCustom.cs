namespace UIWidgets
{
		...
		where TNotification : NotificationCustom<TNotification>
	{
		...
		/// <summary>
		/// Run hide animation on HideButton click.
		/// </summary>
		[Tooltip("Run hide animation on HideButton click.")]
		public bool AnimateOnHideButton = true;
		...
		{
			get => WindowInstances<TNotification>.Instance.Templates;

			set => WindowInstances<TNotification>.Instance.Templates = null;
		}
		...
		/// Animations.
		/// </summary>
		protected IWindowAnimations WindowAnimations;
		...
		/// Buttons pool.
		/// </summary>
		protected ButtonsPool Buttons => buttonsPool ??= new ButtonsPool(this as TNotification, ButtonsTemplates, ButtonsContainer, ButtonsActive, ButtonsCached);
		...
		/// </summary>
		public static IReadOnlyList<TNotification> OpenedNotifications => WindowInstances<TNotification>.Instance.Opened;

		/// <summary>
		/// Inactive notifications with the same template.
		/// </summary>
		public List<TNotification> InactiveNotifications => WindowInstances<TNotification>.Instance.Templates.CachedInstances(TemplateName);

		/// <summary>
		/// All notifications.
		/// </summary>
		public static List<TNotification> AllNotifications => WindowInstances<TNotification>.Instance.All;

		/// <summary>
		/// Count of the opened notifications.
		/// </summary>
		public static int Opened => WindowInstances<TNotification>.Instance.Count;
		...
		{
			add => WindowInstances<TNotification>.Instance.OnOpen += value;

			remove => WindowInstances<TNotification>.Instance.OnOpen -= value;
		}
		...
		{
			add => WindowInstances<TNotification>.Instance.OnClose += value;

			remove => WindowInstances<TNotification>.Instance.OnClose -= value;
		}
		...
		public static void GetOpenedNotifications(List<TNotification> output) => WindowInstances<TNotification>.Instance.GetOpened(output);
		...
		protected virtual void OnDestroy()
		{
			...
			if (!IsTemplate)
			{
				...
				WindowInstances<TNotification>.Instance.Templates = null;
				return;
			}
...
#pragma warning restore 0618
			...
			if (TryGetComponent<IWindowAnimations>(out var animation) && animation.Enabled)
			{
				...
				ShowAnimation = WindowAnimationOpen;
				HideAnimation = WindowAnimationClose;

				#if UNITY_EDITOR
				if ((showAnimation != null) || (hideAnimation != null))
				{
					Debug.LogWarning("Notification.Show: showAnimation and hideAnimation are overridden by " + WindowAnimations, this);
				}
				#endif
			}
...
#pragma warning restore
		...
		{
			NotifyInfo.SetInfo(message, args: null);
		}
		...
		{
			NotifyInfo.SetInfo(message, args: args);
		}
		...
		public override void Display(Action onHideCallback = null)
		{
			...
			if (HideDelay > 0.0f)
			{
				hideCoroutine = HideCoroutine(true);
		...
		protected virtual IEnumerator WindowAnimationOpen(TNotification instance) => WindowAnimations.Open();
		...
		/// <param name="instance">Instance.</param>
		/// <returns>Animation.</returns>
		protected virtual IEnumerator WindowAnimationClose(TNotification instance) => WindowAnimations.Close();
		...
		{
			WindowInstances<TNotification>.Instance.Add(this as TNotification);
		...
		{
			WindowInstances<TNotification>.Instance.Remove(this as TNotification);
		...
		/// Hide coroutine.
		/// </summary>
		/// <param name="withDelay">Hide with delay.</param>
		...
		protected virtual IEnumerator HideCoroutine(bool withDelay = true)
		{
			if (withDelay)
			{
				yield return WidgetsTime.Instance.Wait(HideDelay, UnscaledTime);
			}
			...
			if (HideAnimation != null)
			{
				yield return HideAnimation(this as TNotification);
			}
		...
		{
			if (gameObject.activeInHierarchy && AnimateOnHideButton)
			{
				StopAnimations();
				hideCoroutine = HideCoroutine(false);
				StartCoroutine(hideCoroutine);
			}
			else
			{
