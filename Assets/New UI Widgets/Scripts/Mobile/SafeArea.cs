namespace UIWidgets
{
	...
	public class SafeArea : UIBehaviourInitiable, IUpdatable
	{
		readonly struct ScreenState
		{
			...
			public override int GetHashCode() => HashCode.Combine(SafeArea, Resolution.width, Resolution.height, Orientation);
		...
		public Borders GetBorders() => new Borders(canvas, lastState.SafeArea);
		...
		void ChangeScreenState()
		{
			...
			OnScreenChange.Invoke(GetBorders());
		}
	}
}