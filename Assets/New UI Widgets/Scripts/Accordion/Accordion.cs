namespace UIWidgets
{
	...
	public class Accordion : UIBehaviourInteractable, IStylable
	{
		...
		protected virtual IEnumerator OpenCoroutine(AccordionItem item)
		{
			...
			if (ResizeMethod == ResizeMethods.Size)
			{
				yield return StartCoroutine(AnimationOpen(item.ContentObjectRect, AnimationDuration, Curve, IsHorizontal(), UnscaledTime, StaticFields.Instance.DoNothing));
			}
			else if (ResizeMethod == ResizeMethods.Flexible)
			{
				yield return StartCoroutine(AnimationOpenFlexible(item.ContentObjectRect, AnimationDuration, Curve, IsHorizontal(), UnscaledTime, StaticFields.Instance.DoNothing));
			}
		...
		protected virtual IEnumerator HideCoroutine(AccordionItem item)
		{
			...
			if (ResizeMethod == ResizeMethods.Size)
			{
				yield return StartCoroutine(AnimationClose(item.ContentObjectRect, AnimationDuration, Curve, IsHorizontal(), UnscaledTime, StaticFields.Instance.DoNothing));
			}
			else if (ResizeMethod == ResizeMethods.Flexible)
			{
				yield return StartCoroutine(AnimationCloseFlexible(item.ContentObjectRect, AnimationDuration, Curve, IsHorizontal(), UnscaledTime, StaticFields.Instance.DoNothing));
			}
