	...
	public class LineBuilder : ILineBuilder
	{
		...
		public virtual int Build(ConnectorBase connector, RectTransform source, ConnectorLine line, VertexHelper vh, int index)
		{
			...
			switch (line.Type)
			{
				...
				case ConnectorType.Rectangular:
					...
					break;
				default:
					throw new NotSupportedException(string.Format("Unsupported line type: {0}", EnumHelper<ConnectorType>.Instance.ToString(line.Type)));
			}

			if (EnumHelper<ConnectorArrow>.Instance.HasFlag(line.Arrow, ConnectorArrow.Forward))
			{
				points += AddArrow(Points[Points.Count - 2], Points[Points.Count - 1], line.ArrowSize, VertexIndex + points);
			}

			if (EnumHelper<ConnectorArrow>.Instance.HasFlag(line.Arrow, ConnectorArrow.Backward))
			{
		...
		protected int RectangularStart(ConnectorPosition start, Vector3 startPoint, Vector3 endPoint)
		{
			return start switch
			{
				...
				_ => throw new NotSupportedException(string.Format("Unsupported line start: {0}", EnumHelper<ConnectorPosition>.Instance.ToString(start))),
			};
		}
		...
		protected int RectangularLeft(Vector3 startPoint, Vector3 endPoint)
		{
			...
			switch (end)
			{
				...
				case ConnectorPosition.Bottom:
					...
					break;
				default:
					throw new NotSupportedException(string.Format("Unsupported line end: {0}", EnumHelper<ConnectorPosition>.Instance.ToString(Line.End)));
			}
