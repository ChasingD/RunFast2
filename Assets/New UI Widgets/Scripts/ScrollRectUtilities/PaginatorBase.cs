namespace UIWidgets
{
	...
	public abstract partial class PaginatorBase : UIBehaviourInteractable, IStylable
	{
		...
		protected virtual void CalculateVisiblePages(int page)
		{
			...
			var half = Mathf.Max(0, Mathf.FloorToInt(VisiblePagesCount / 2f));
			var visible = (half * 2) + 1;
			var min = Mathf.Max(0, page - half);
			var max = Mathf.Min(Pages - 1, min + visible);

			if ((max > visible) && ((max - min) != visible))
			{
				min = Mathf.Max(0, max - visible + 1);
			}
