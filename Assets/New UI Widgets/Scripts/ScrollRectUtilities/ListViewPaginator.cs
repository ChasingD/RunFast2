namespace UIWidgets
{
	...
	public class ListViewPaginator : ScrollRectPaginator
	{
		...
		public override float Page2Position(int page)
		{
			...
			var result = ForcedPosition switch
			{
				PaginatorPagePosition.None => ListView.GetItemPosition(index),
				PaginatorPagePosition.OnStart => ListView.GetItemPosition(index),
				PaginatorPagePosition.OnCenter => ListView.GetItemPositionMiddle(index),
				PaginatorPagePosition.OnEnd => ListView.GetItemPositionBottom(index),
				_ => throw new NotSupportedException(string.Format("Unknown forced position: {0}", EnumHelper<PaginatorPagePosition>.Instance.ToString(ForcedPosition))),
			};
