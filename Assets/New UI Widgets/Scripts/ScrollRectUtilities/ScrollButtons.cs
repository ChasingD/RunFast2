namespace UIWidgets
{
	...
	public class ScrollButtons : MonoBehaviourInitiable
	{
		...
		public void ScrollOnHold(ScrollButtonType type)
		{
			...
			AnimationStartTime = WidgetsTime.Instance.Time(UnscaledTime);
		...
		public void ScrollOnClick(ScrollButtonType type)
		{
			...
			if ((AnimationStartTime + AnimationLength) < WidgetsTime.Instance.Time(UnscaledTime))
			{
				return;
			}
		...
		protected virtual IEnumerator AnimationHold(ScrollButtonType type)
		{
			do
			{
				var delta = WidgetsTime.Instance.DeltaTime(UnscaledTime);
		...
		protected virtual Vector3 ScrollOnClick(ScrollButtonType type, Vector3 position, float value)
		{
			switch (type)
			{
				...
				case ScrollButtonType.Right:
					...
					break;
				default:
					throw new NotSupportedException(string.Format("Unknown ScrollButtonType: {0}", EnumHelper<ScrollButtonType>.Instance.ToString(type)));
			}
