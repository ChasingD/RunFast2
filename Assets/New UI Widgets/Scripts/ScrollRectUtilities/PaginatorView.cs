namespace UIWidgets
{
	...
	public abstract partial class PaginatorBase : UIBehaviourInteractable, IStylable
	{
		...
		public class PaginatorView
		{
			...
			{
				DefaultPages.RemoveAll(StaticFields.Instance.PaginatorIsNullComponent);
			...
			{
				var has_active_page = activePage != null;
				...
				if (defaultPagesCache.Template != null)
				{
					...
					if (has_active_page)
					{
						...
						if (skipPagesCache.Template != null)
						{
							...
							{
								var visible_page = Owner.VisiblePages[i];
								var diff = visible_page - prev;
								if (diff > 1)
								{
									var rt = visible_page == page ? activePage.RectTransform : DefaultPages[i].RectTransform;
									SetSkipPage(rt);
								}
				...
				if (PagesContainer != null)
				{
					LayoutRebuilder.ForceRebuildLayoutImmediate(PagesContainer);
				}
			}
