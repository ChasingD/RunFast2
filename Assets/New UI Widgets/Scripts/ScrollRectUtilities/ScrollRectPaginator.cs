namespace UIWidgets
{
	...
	public class ScrollRectPaginator : PaginatorBase, IUpgradeable
	{
		...
		protected virtual void OnScrollRectDragStart(PointerEventData eventData)
		{
			...
			DragStarted = WidgetsTime.Instance.Time(UnscaledTime);
		...
		protected int Rounding(float page)
		{
			return PageRounding switch
			{
				...
				_ => throw new NotSupportedException(string.Format("Unknown PageRounding: {0}", EnumHelper<PaginatorRounding>.Instance.ToString(PageRounding))),
			};
		}
		...
		protected virtual void ScrollChanged()
		{
			...
			var time = WidgetsTime.Instance.Time(UnscaledTime) - DragStarted;
		...
		public virtual float Page2Position(int page)
		{
			...
			switch (ForcedPosition)
			{
				...
				case PaginatorPagePosition.OnEnd:
					...
					break;
				default:
					throw new NotSupportedException(string.Format("Unknown forced position: {0}", EnumHelper<PaginatorPagePosition>.Instance.ToString(ForcedPosition)));
			}
		...
		protected virtual float ClampPosition(float position)
		{
