		...
		where TItemView : ListViewItem
	{
		...
		protected class TileViewTypeSize : ListViewTypeSize
		{
			...
			protected override int CalculateMaxVisibleItems(ObservableList<TItem> items)
			{
				...
				{
					Rows = MaxVisible(false, height, Owner.GetItemSpacingY(), false);
				...
				{
					Columns = MaxVisible(true, width, Owner.GetItemSpacingX(), false);
			...
			public override float GetItemPositionBottom(int index)
			{
				...
				return size + (Owner.LayoutBridge.GetSpacing() * (block - 1)) + Owner.LayoutBridge.GetMargin(Owner.ReversedOrder) - Owner.Viewport.ScaledAxisSize;
			}
