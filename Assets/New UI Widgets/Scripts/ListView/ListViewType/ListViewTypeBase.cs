namespace UIWidgets
{
		...
		where TItemView : ListViewItem
	{
		...
		protected abstract class ListViewTypeBase
		{
			...
			/// <summary>
			/// Is bottom border of item is visible.
			/// </summary>
			/// <param name="index">Index.</param>
			/// <returns>true if bottom border of the item is visible; false otherwise.</returns>
			public abstract bool IsBottomBorderVisible(int index);
			...
			{
				// Important: Owner.LayoutBridge.GetFullMarginX() should not deduced, because it not limits visible area
			...
			{
				// Important: Owner.LayoutBridge.GetFullMarginY() should not deduced, because it not limits visible area
