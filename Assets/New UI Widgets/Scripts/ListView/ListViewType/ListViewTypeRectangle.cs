namespace UIWidgets
{
		...
		where TItemView : ListViewItem
	{
		...
		protected abstract class ListViewTypeRectangle : ListViewTypeBase
		{
			...
			{
				return GetPosition() + (Owner.Viewport.ScaledAxisSize / 2f) - Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
			}
			...
			public override bool IsVisible(int index, float minVisiblePart)
			{
				...
				var viewport_top = GetPosition() + Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
			...
			/// <inheritdoc/>
			public override bool IsBottomBorderVisible(int index)
			{
				if (!Owner.IsValid(index))
				{
					return false;
				...
				return (viewport_top <= border_bottom) && (border_bottom <= viewport_bottom);
			}
