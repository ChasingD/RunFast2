		...
		protected class TileViewStaggered : ListViewTypeSize
		{
			...
			protected override int CalculateMaxVisibleItems(ObservableList<TItem> items)
			{
				...
				var max_items_per_block = Owner.IsHorizontal()
					? MaxVisible(true, width, spacing_x, false) + 1
					: MaxVisible(false, height, spacing_y, false) + 1;
			...
			public override float GetItemPosition(int index)
			{
				...
				return size + Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
			}
			...
			public override float GetItemPositionBottom(int index)
			{
				...
				return size + Owner.LayoutBridge.GetMargin(Owner.ReversedOrder) - Owner.Viewport.ScaledAxisSize;
			}
			...
			public override bool UpdateDisplayedIndices()
			{
				...
				var start = Mathf.Max(0f, GetPosition() - Owner.LayoutBridge.GetMargin(Owner.ReversedOrder));
			...
			public override int GetNearestIndex(Vector2 point, NearestType type)
			{
				...
				var item_pos = (Owner.IsHorizontal() ? point.x : Mathf.Abs(point.y)) - Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
