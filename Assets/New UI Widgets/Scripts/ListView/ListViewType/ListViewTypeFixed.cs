		...
		where TItemView : ListViewItem
	{
		...
		protected class ListViewTypeFixed : ListViewTypeRectangle
		{
			...
			public override float GetItemPosition(int index)
			{
				...
				return centered_filler + (block_index * GetItemSize()) + Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
			}
			...
			public override int GetNearestIndex(Vector2 point, NearestType type)
			{
				...
				pos -= Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
				...
				var index = type switch
				{
					...
					_ => throw new NotSupportedException(string.Format("Unsupported NearestType: {0}", EnumHelper<NearestType>.Instance.ToString(type))),
				};
