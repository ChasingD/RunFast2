		...
		where TItemView : ListViewItem
	{
		...
		protected class ListViewTypeSize : ListViewTypeRectangle
		{
				...
				: base(owner)
			{
				...
				foreach (var template in Owner.DefaultTemplateSelector.AllTemplates())
				{
					if (template.GetComponentInChildren<ContentSizeFitter>(includeInactive: true) != null)
					{
						Debug.LogWarning("Avoid using ContentSizeFitter on a GameObject that is also inside a LayoutGroup. This can cause layout conflicts and unpredictable UI behavior.", template);
					}
				}
			}
			...
			/// <param name="smallestSize">Use smallest size.</param>
			/// <returns>Maximum visible items count.</returns>
			protected int MaxVisible(bool isHorizontal, float visibleAreaSize, float spacing, bool smallestSize)
			{
				return InstanceSizes.Visible(Owner.DataSource, isHorizontal, visibleAreaSize, spacing, smallestSize);
			}
			...
			protected virtual int CalculateMaxVisibleItems(ObservableList<TItem> items)
			{
				...
				var visible = MaxVisible(Owner.IsHorizontal(), Owner.Viewport.ScaledAxisSize, Owner.LayoutBridge.GetSpacing(), true);
			...
			{
				return GetItemPosition(Visible.FirstVisible) - Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
			}
			...
			{
				var pos = GetPosition() - Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
			...
			{
				var pos = Mathf.Max(0f, GetPosition() - Owner.LayoutBridge.GetMargin(Owner.ReversedOrder));
			...
			public override float GetItemPosition(int index)
			{
				...
				return size + (Owner.LayoutBridge.GetSpacing() * index) + Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
			}
			...
			public override float GetItemPositionBottom(int index)
			{
				...
				return GetItemPosition(index) + GetInstanceSize(index) + Owner.LayoutBridge.GetMargin(Owner.ReversedOrder) - Owner.LayoutBridge.GetSpacing() - Owner.Viewport.ScaledAxisSize;
			}
			...
			int GetIndexAtPosition(float position, NearestType type)
			{
				...
				switch (type)
				{
					...
					case NearestType.After:
						...
						break;
					default:
						throw new NotSupportedException(string.Format("Unsupported NearestType: {0}", EnumHelper<NearestType>.Instance.ToString(type)));
				}
			...
			protected virtual float ScrollPositionCompensation(TItem resizedItem, float newSize)
			{
				...
				var base_viewport_top = GetPosition(Owner.ContainerRealAnchoredPosition) - Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
				...
				for (var i = 0; i < Owner.DataSource.Count; i++)
				{
					...
					var same_item = EqualityComparer<TItem>.Default.Equals(resizedItem, item);
					if (!same_item)
					{
						continue;
					}
			...
			public override int GetNearestIndex(Vector2 point, NearestType type)
			{
				...
				pos_block -= Owner.LayoutBridge.GetMargin(Owner.ReversedOrder);
