namespace UIWidgets
{
		...
		where TItemView : ListViewItem
	{
		...
		/// </summary>
		protected virtual ListViewComponentPool ComponentsPool => componentsPool ??= new ListViewComponentPool(this, SetInstanceData);
		...
		/// </summary>
		public Dictionary<InstanceID, Vector2> OverriddenTemplateSizes => overriddenTemplateSizes ??= new Dictionary<InstanceID, Vector2>();
		...
		{
			static bool IsNullTemplate(Template x) => x.Template == null;
			...
			Instances.RemoveAll(StaticFields.Instance.ListViewItemIsNull);

			OwnTemplates.RemoveAll(IsNullTemplate);
			...
			if (SharedTemplates != null)
			{
				SharedTemplates.RemoveAll(IsNullTemplate);
		...
		protected virtual ListViewTypeBase GetRenderer(ListViewType type)
		{
			ListViewTypeBase renderer = type switch
			{
				...
				_ => throw new NotSupportedException(string.Format("Unknown ListView type: {0}", EnumHelper<ListViewType>.Instance.ToString(type))),
			};
...
#pragma warning restore 0618
		...
		/// </summary>
		protected int PreviousItemsCount = 0;
		...
		protected virtual void SetNewItems(ObservableList<TItem> newItems, bool updateView = true)
		{
			...
			lock (lockObject)
			{
				var at_bottom = KeepScrollAtBottom && (PreviousItemsCount > 0) && ListRenderer.IsBottomBorderVisible(PreviousItemsCount - 1);
				...
				if (different)
				{
					newItems.OnChangeMono.AddListener(UpdateItems);
				}

				PreviousItemsCount = newItems.Count;
		...
		/// <inheritdoc/>
		public override void SetTableSizes(IList<TableHeader.CellSizeData> sizes)
		{
			var width = !IsHorizontal();
			var axis = width ? RectTransform.Axis.Horizontal : RectTransform.Axis.Vertical;
			foreach (var instance in GetComponentsEnumerator(PoolEnumeratorMode.All))
			{
				for (int i = 0; i < sizes.Count; i++)
				{
					...
					if (!cell.TryGetComponent<LayoutElement>(out var le))
					{
						cell.SetSizeWithCurrentAnchors(axis, data.Size);
						continue;
					}

					if (width)
					{
						if (data.LayoutElement != null)
						{
							le.minWidth = data.LayoutElement.minWidth;
							le.flexibleWidth = data.LayoutElement.flexibleWidth;
						}

						le.preferredWidth = data.Size;
					}
					else
					{
						if (data.LayoutElement != null)
						{
							le.minHeight = data.LayoutElement.minHeight;
							le.flexibleHeight = data.LayoutElement.flexibleHeight;
						}

						le.preferredHeight = data.Size;
					}
				}
			}
		}
		...
		{
			LastScrollingTime = WidgetsTime.Instance.Time(true);
			if (Scrolling)
			{
				return;
			}
		...
		protected virtual bool IsStopScrolling()
		{
			...
			return (LastScrollingTime + EndScrollDelay) <= WidgetsTime.Instance.Time(true);
		}
		...
		/// AutoScroll.
		/// </summary>
		protected virtual ListViewAutoScroll AutoScroll => autoScroll ??= new ListViewAutoScroll(this, ScrollRect.transform as RectTransform);
