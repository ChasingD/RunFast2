...
namespace UIWidgets
{
			...
			#endif
	{
		...
		/// <summary>
		/// Keep scroll at the bottom on DataSource change (only if scroll already at bottom).
		/// </summary>
		[SerializeField]
		[Tooltip("Keep scroll at the bottom on DataSource change (only if scroll already at bottom).")]
		public bool KeepScrollAtBottom = false;
		...
		/// </summary>
		public Func<int, bool> CanSelect = StaticFields.Instance.ListViewAlwaysAllow;

		/// <summary>
		/// Determinate if index can be deselected.
		/// </summary>
		public Func<int, bool> CanDeselect = StaticFields.Instance.ListViewAlwaysAllow;
		...
		protected override void InitOnce()
		{
			...
			NotIsValid = index => !IsValid(index);
		...
		/// </summary>
		protected ListViewItem CurrentPointerHighlightedItem => GetInstance(IndexUnderPointer);
		...
		/// Items under pointer.
		/// </summary>
		protected static ItemsUnderPointerFinder ItemsUnderPointer => StaticFields.Instance.ListViewItemsUnderPointer;
		...
		protected virtual void SilentDeselect(List<int> indices)
		{
			...
			// call events only after all indices removed to prevent problems with invalid selected indices after items were removed
			foreach (var index in indices)
			{
				OnDeselectInternal.Invoke(index);
			}
		...
		/// </summary>
		/// <param name="sizes">Column or rows sizes (columns if Horizontal direction, rows otherwise).</param>
		public virtual void SetTableSizes(IList<TableHeader.CellSizeData> sizes)
		{
			throw new NotSupportedException();
		}
