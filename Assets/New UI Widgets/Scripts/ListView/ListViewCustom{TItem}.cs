namespace UIWidgets
{
	...
	public abstract class ListViewCustom<TItem> : ListViewCustomBase
	{
		...
		public virtual void Select(TItem item, bool raiseEvents = true)
		{
			Select(DataSource.IndexOf(item), raiseEvents);
		}
		...
		{
			#if UNITY_EDITOR
			if (!allowDuplicate && !DataTypeInfo<TItem>.Instance.IsEquatable)
			{
				var msg = string.Format("Type {0} must implement the IEquatable<T> interface to check item equality. Otherwise, the AllowDuplicate option will not work correctly.", UtilitiesEditor.GetFriendlyTypeName(typeof(TItem)));
