namespace UIWidgets
{
	...
	public class EasyLayoutBridge : ILayoutBridge
	{
		...
		/// <summary>
		/// Gets the margin by direction and reversed order.
		/// </summary>
		/// <param name="reversedOrder">Reversed order.</param>
		/// <returns>The margin.</returns>
		public float GetMargin(bool reversedOrder)
		{
			if (reversedOrder)
			{
				return IsHorizontal ? Layout.GetMarginRight() : Layout.GetMarginBottom();
			}
