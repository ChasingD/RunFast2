namespace UIWidgets
{
		...
		#endif
	{
		...
		readonly Func<long, T> DefaultLong2Enum = value => (T)Enum.ToObject(typeof(T), value);

		readonly Func<T, long> Enum2Long;

		readonly Func<T, long> DefaultEnum2Long = value => Convert.ToInt64(value);
		...
		public ListViewEnum(ListViewEnum listView, bool showObsolete = false, Func<long, T> long2enum = null, Func<T, long> enum2long = null)
		{
			...
			ListView.DeselectAll();
			ListView.MultipleSelect = EnumHelper<T>.Instance.IsFlags;

			var items = ListView.DataSource;
			items.Clear();
			using var _ = items.BeginUpdate();

			var values = EnumHelper<T>.Instance.ValuesLong;
			var names = EnumHelper<T>.Instance.Names;
			var obsolete = EnumHelper<T>.Instance.Obsolete;
			...
			for (int i = 0; i < values.Length; i++)
			{
				...
				items.Add(new ListViewEnum.Item(values[i], names[i], obsolete[i]));
			}
		}
		...
		/// <param name="long2enum">Long to enum converter.</param>
		/// <param name="enum2long">Enum to long converter.</param>
		public ListViewEnum(ListViewEnum listView, IList<T> allowedValues, Func<long, T> long2enum = null, Func<T, long> enum2long = null)
		{
#if !CSHARP_7_3_OR_NEWER
			var type = typeof(T);
			if (!type.IsEnum)
			...
			var values_long = EnumHelper<T>.Instance.ValuesLong;
			var names = EnumHelper<T>.Instance.Names;
			var values = EnumHelper<T>.Instance.Values;
			var obsolete = EnumHelper<T>.Instance.Obsolete;

			for (int i = 0; i < values_long.Length; i++)
			{
				if (!allowedValues.Contains(values[i]))
				{
					continue;
				}

				items.Add(new ListViewEnum.Item(values_long[i], names[i], obsolete[i]));
			}
		}
	}
}