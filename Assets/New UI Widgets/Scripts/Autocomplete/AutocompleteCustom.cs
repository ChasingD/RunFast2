namespace UIWidgets
{
		...
		where TListViewComponent : ListViewItem
	{
		...
		protected override void InitOnce()
		{
			...
			#if UNITY_EDITOR
			if (!DataTypeInfo<TValue>.Instance.IsEquatable)
			{
				var msg = string.Format("Type {0} must implement the IEquatable<T> interface to check item equality. Otherwise, the AllowDuplicate option will not work correctly.", UtilitiesEditor.GetFriendlyTypeName(typeof(TValue)));
		...
		protected virtual IEnumerator Search()
		{
			if (SearchDelay > 0)
			{
				yield return WidgetsTime.Instance.Wait(SearchDelay, UnscaledTime);
			}
