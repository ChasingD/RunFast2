namespace UIWidgets
{
		...
		where TCombobox : ComboboxCustom<TListView, TListViewComponent, TItem>
	{
		...
		/// </summary>
		[SerializeField]
		[Tooltip("Requires overridden Input2Item method.")]
		...
		/// </summary>
		[SerializeField]
		public ListViewCustom<TListViewComponent, TItem>.ItemEvent OnItemSelected = new ListViewCustom<TItem>.ItemEvent();

		/// <summary>
		/// On item deselected event.
		/// Item is not valid if list is cleared or selected item was removed.
		/// </summary>
		[SerializeField]
		public ListViewCustom<TListViewComponent, TItem>.ItemEvent OnItemDeselected = new ListViewCustom<TItem>.ItemEvent();
		...
		protected override void InitOnce()
		{
			...
			Combobox.ListView.OnSelectInternal.AddListener(OnSelectIndexChanged);
			Combobox.ListView.OnDeselectInternal.AddListener(OnSelectIndexChanged);
			Combobox.OnCurrentClick.AddListener(ComboboxCurrentClick);
			Combobox.ListView.OnItemSelected.AddListener(ProcessItemSelected);
			Combobox.ListView.OnItemDeselected.AddListener(ProcessItemDeselected);
		...
		/// <summary>
		/// Process ItemSelected event.
		/// </summary>
		/// <param name="index">Selected index.</param>
		/// <param name="item">Selected item.</param>
		protected virtual void ProcessItemSelected(int index, TItem item) => OnItemSelected.Invoke(index, item);

		/// <summary>
		/// Process ItemDeselected event.
		/// </summary>
		/// <param name="index">Deselected index.</param>
		/// <param name="item">Deselected item.</param>
		protected virtual void ProcessItemDeselected(int index, TItem item) => OnItemDeselected.Invoke(index, item);

		/// <summary>
		/// Deselect all items.
		/// </summary>
		/// <param name="raiseEvents">Raise select events?</param>
		public virtual void DeselectAll(bool raiseEvents = true) => Combobox.ListView.DeselectAll(raiseEvents);
		...
		protected virtual void OnDestroy()
		{
			...
			if (Combobox != null)
			{
				Combobox.ListView.OnSelectInternal.RemoveListener(OnSelectIndexChanged);
				Combobox.ListView.OnDeselectInternal.RemoveListener(OnSelectIndexChanged);
				Combobox.ListView.OnItemSelected.RemoveListener(ProcessItemSelected);
				Combobox.ListView.OnItemDeselected.RemoveListener(ProcessItemDeselected);
		...
		protected virtual void OnOptionSelected(TItem item) => Combobox.ListView.Select(item);
		...
		protected virtual void ProcessItemNotFound(string input)
		{
			...
			if (Combobox.ListView.IsValid(index) && !Combobox.ListView.IsSelected(index))
			{
		...
		/// </summary>
		protected virtual void ProcessCancel() => AutocompleteHide(false);
		...
		/// </summary>
		/// <param name="ignore">Index.</param>
		protected virtual void OnSelectIndexChanged(int ignore)
		{
			if (HasSelected())
			{
				Autocomplete.InputFieldAdapter.text = GetStringValue(SelectedItem);
				AutocompleteHide(false);
			}
			else
			{
		...
		protected bool HasSelected() => Combobox.ListView.SelectedIndex >= 0;
		...
		protected virtual void AutocompleteShow(bool setFocus)
		{
			if (HasSelected())
			{
			...
			if (setFocus && !EventSystem.current.alreadySelecting)
			{
		...
		protected void InputFocusLost(BaseEventData eventData) => AutocompleteHide(false);
		...
		protected virtual void AutocompleteHide(bool requireSelected)
		{
			...
			{
				AutocompleteShow(requireSelected);
			}
		}
